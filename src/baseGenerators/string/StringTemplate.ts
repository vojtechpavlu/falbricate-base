...
export interface StringTemplateVariables {
  [variable: string]: (DeclarativeFieldDefinition | ValueGenerator<GeneratedValue, ValueGeneratorConfig>);
}
...
> {
  ...
  protected get = (context?: FabricationContext): GeneratedValue => {
    ...
    const generatedVariables = this.fabricator.generate(context) as ObjectFalsum;
    ...
    templateString = this.replaceCustomCharsets(templateString, this.config.customCharsets!);
  ...
  private replaceCustomCharsets = (template: string, customCharsets: CustomCharsets) => {
  ...
  private replaceNumerics = (template: string) => {
    ...
    template = template.replace(new RegExp('%d', 'g'), () => randomInteger(0, 9).toString())

    // Replace %D (numbers in range [1, 9])
    template = template.replace(new RegExp('%D', 'g'), () => randomInteger(1, 9).toString())
  ...
  private replaceCharacters = (template: string) => {
    ...
    template = template.replace(new RegExp('%c', 'g'), () => randomCharacter(lowercase_chars));

    // Replace %C (ASCII characters like [A-Z])
    const uppercase_chars = getCharset('uppercase');
    template = template.replace(new RegExp('%C', 'g'), () => randomCharacter(uppercase_chars));
